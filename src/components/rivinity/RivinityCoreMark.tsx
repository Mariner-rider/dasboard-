...
/**
 * RivinityCoreMark — reusable brand primitive.
 *
 * Renders the actual Rivinity lotus geometry (two paths copied verbatim
 * from the uploaded logo SVG, identical to the "Born from motion"
 * animation) as a low-opacity, dashed, gradient stroke. It is NOT
 * decorative — every capability surrounding the mark maps to a slice
 * of the outline. When `activeIndex` is set, that petal slice lights up
 * with the full Intelligence Flow gradient, so the mark gradually comes
 * alive as visitors explore the platform.
 *
 * Reuse anywhere the Rivinity architecture is explained.
 */
...
const GRADIENT_STOPS: { offset: string; color: string }[] = [
  { offset: "0%",   color: "#FD881F" },
  { offset: "35%",  color: "#F5A9D0" },
  { offset: "70%",  color: "#D8A5F2" },
  { offset: "100%", color: "#BFA7F8" },
];

/* Canvas we render into. Bounding box of both paths is computed once
   in-browser and fit into this square. */
const VB = 600;
const FIT = 520;

type Fit = { scale: number; tx: number; ty: number; lengths: number[] };

let _cachedFit: Fit | null = null;

function computeFit(): Fit | null {
  if (typeof document === "undefined") return null;
  if (_cachedFit) return _cachedFit;
  ...
  const tx = VB / 2 - cx * scale;
  const ty = VB / 2 - cy * scale;

  _cachedFit = { scale, tx, ty, lengths };
  return _cachedFit;
}

export interface RivinityCoreMarkProps {
  /** Total number of capability segments the mark subdivides into. */
  segments?: number;
  /** Which segment index (0..segments-1) is currently active; null hides. */
  activeIndex?: number | null;
  /** Rendered size in px (square). */
  size?: number;
  className?: string;
  /** Whether to show the caption pill under the mark. */
  label?: string | null;
  /** Unique id suffix — required if multiple marks render on the same page. */
  idSuffix?: string;
}

/**
 * A single reusable Rivinity core mark. Renders the lotus outline at very
 * low opacity as a thin dashed stroke, plus a hidden overlay path that
 * reveals exactly one 1/N slice of the outline when a capability is
 * hovered — mapping each surrounding module to a specific petal.
 */
export const RivinityCoreMark = ({
  segments = 15,
  activeIndex = null,
  size = 220,
  className,
  label = null,
  idSuffix = "core",
}: RivinityCoreMarkProps) => {
  const [fit, setFit] = useState<Fit | null>(() => _cachedFit);
  useEffect(() => {
    if (fit) return;
    // schedule after paint so we don't block first render
    const id = requestAnimationFrame(() => setFit(computeFit()));
    return () => cancelAnimationFrame(id);
  }, [fit]);

  const gradId = `rcm-grad-${idSuffix}`;
  const glowId = `rcm-glow-${idSuffix}`;

  /* Distribute segments across the two paths proportional to length. */
  const segMap = useMemo(() => {
    const lens = fit?.lengths ?? [1, 1];
    const total = lens[0] + lens[1];
    const nA = Math.round((lens[0] / total) * segments);
    const nB = segments - nA;
    return [nA, nB];
  }, [fit, segments]);

  const active = activeIndex ?? -1;
  const activePath = active >= 0 && active < segMap[0] ? 0 : 1;
  const localIdx  = active >= 0 && active < segMap[0] ? active : active - segMap[0];
  const localSegs = segMap[activePath] || 1;
  ...
  return (
    <div
      ...
      aria-hidden
    >
      {/* Ambient glow only — no new tokens */}
      <div
        className="absolute inset-[8%] rounded-full blur-2xl"
        style={{
          background:
            "radial-gradient(circle, rgba(253,136,31,0.14), rgba(191,167,248,0.08) 55%, transparent 75%)",
        }}
      />
      <svg
        viewBox={`0 0 ${VB} ${VB}`}
      ...
      >
        ...
        {fit && (
          <g transform={`translate(${fit.tx.toFixed(3)} ${fit.ty.toFixed(3)}) scale(${fit.scale.toFixed(6)})`}>
            {/* Base outline — dashed, low-opacity, gradient stroke */}
            {RAW_PATHS.map((rp, i) => (
              <path
                key={`base-${i}`}
                d={rp.d}
                ...
                strokeDasharray={`${3 / fit.scale} ${3.5 / fit.scale}`}
                strokeLinecap="round"
            ...
            {/* Highlight overlay — reveals exactly one 1/N slice of the
                active petal path when a capability is hovered. */}
            {RAW_PATHS.map((rp, i) => {
              const isActivePath = i === activePath;
              const show = active >= 0 && isActivePath;
              return (
                <path
                  key={`hi-${i}`}
                  d={rp.d}
                  ...
                  fill="none"
                  stroke={`url(#${gradId})`}
                  strokeWidth={2.4 / fit.scale}
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  pathLength={localSegs}
                  strokeDasharray={show ? `1 ${localSegs - 1}` : "0 1"}
                  strokeDashoffset={show ? -localIdx : 0}
                  opacity={show ? 0.9 : 0}
                  filter={`url(#${glowId})`}
                  vectorEffect="non-scaling-stroke"
                  style={{
                    transition: "opacity 500ms ease, stroke-dashoffset 700ms cubic-bezier(0.22,1,0.36,1)",
                  }}
                />
              );
            })}
...
/**
 * Compute where a spoke should terminate on the mark's perimeter for a
 * given angle (radians, 0 = east, -PI/2 = north). Returns a point in
 * arbitrary units — callers scale it to their own coordinate system.
 * Approximates the lotus outline as an ellipse whose axes match the fit.
 */
export const rivinityMarkPerimeter = (angle: number, rx = 1, ry = 1) => ({
  x: rx * Math.cos(angle),
  y: ry * Math.sin(angle),
});

export default RivinityCoreMark;