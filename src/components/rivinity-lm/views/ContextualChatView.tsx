...
import { Send, Paperclip, Sparkles, Loader2, BookOpen, Lightbulb, Copy, ThumbsUp, ThumbsDown, Share2, RefreshCw, Volume2, Pencil } from "lucide-react";
...
const ContextualChatView = () => {
  ...
  return (
    <div className="flex flex-col h-full max-w-[800px] mx-auto">
      {/* Important Topics sidebar floating */}
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className="group max-w-[85%]">
                ...
                {!msg.thinking && (
                  <div className={`flex items-center gap-0.5 mt-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                    {msg.role === "user" ? (
                      <>
                        <button onClick={() => { navigator.clipboard.writeText(msg.content); toast.success("Copied!"); }} className="p-1.5 rounded-lg hover:bg-accent/50 transition-colors text-muted-foreground/40 hover:text-muted-foreground/70" title="Copy">
                          <Copy className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => setInput(msg.content)} className="p-1.5 rounded-lg hover:bg-accent/50 transition-colors text-muted-foreground/40 hover:text-muted-foreground/70" title="Edit">
                      ...
                      <>
                        ...
                        <button className="p-1.5 rounded-lg hover:bg-accent/50 transition-colors text-muted-foreground/40 hover:text-green-500/70" title="Good response">
                          <ThumbsUp className="w-3.5 h-3.5" />
                        </button>
                        <button className="p-1.5 rounded-lg hover:bg-accent/50 transition-colors text-muted-foreground/40 hover:text-red-400/70" title="Bad response">
                          <ThumbsDown className="w-3.5 h-3.5" />
                        </button>
                        <button className="p-1.5 rounded-lg hover:bg-accent/50 transition-colors text-muted-foreground/40 hover:text-muted-foreground/70" title="Share">
                          <Share2 className="w-3.5 h-3.5" />
                        </button>
                        <button className="p-1.5 rounded-lg hover:bg-accent/50 transition-colors text-muted-foreground/40 hover:text-muted-foreground/70" title="Regenerate">
                          <RefreshCw className="w-3.5 h-3.5" />
                        </button>
                        <button className="p-1.5 rounded-lg hover:bg-accent/50 transition-colors text-muted-foreground/40 hover:text-muted-foreground/70" title="Read aloud">
