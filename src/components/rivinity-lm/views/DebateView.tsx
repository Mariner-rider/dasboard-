    ...
    return (
      <div className="max-w-[600px] mx-auto">
        ...
        <div className="glass rounded-2xl border border-glass p-5 mb-6 shadow-float">
          <p className="text-[12px] font-semibold text-foreground/70 mb-3">Choose a Topic</p>
          ...
          <p className="text-[11px] text-muted-foreground/40 mb-2">Or enter your own:</p>
          <input value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="Enter a debate topic..."
            className="w-full bg-muted/30 rounded-xl px-4 py-2.5 text-[12px] text-foreground placeholder:text-muted-foreground/40 focus:outline-none mb-4" />
        </div>
        ...
        <button onClick={() => topic && startDebate(topic)} disabled={!topic}
          className="w-full py-3.5 rounded-xl gradient-accent text-primary-foreground text-[14px] font-semibold hover:opacity-90 transition-opacity disabled:opacity-40">
          Start Debate
        </button>
      </div>
    );
